	...
	public class Calculator {
		...
		public Calculator(EWAHCompressedBitmap targetProfileBM, EWAHCompressedBitmap negatedTargetProfileBM) {
			...
			if (!targetToQueryCache.containsKey(targetProfilePair)) {
				targetToQueryCache.put(targetProfilePair, new NodeProbabilities[knowledgeBase.getNumClassNodes()]);
			}
			// probCache = new
			// NodeProbabilities[getKnowledgeBase().getNumClassNodes()];
		}
		 ...
		 * @throws IncoherentStateException
		 */
		public double calculateProbability(Set<String> queryClassIds, Set<String> negatedQueryClassIds)
				throws IncoherentStateException {
			double cump = 1.0;
		...
		private NodeProbabilities calculateProbability(int qcix) throws IncoherentStateException {
			...
			if (targetProfileBM.get(qcix)) {
				...
				prdChild = new NodeProbabilities(0.95, 0.01); // TODO - do not
																// hardcode
			} else if (negatedTargetProfileBM != null && negatedTargetProfileBM.get(qcix)) {
				LOG.info("Q is in negative target profile");
				prdChild = new NodeProbabilities(0.01, 0.99); // TODO - do not
																// hardcode
			} else {
				...
				for (int i = 0; i < pixs.size(); i++) {
					...
					// recursive call
					parentOnProbs[i] = calculateProbability(pixs.get(i));
				}
				...
				for (int parentsStateComboIx = 0; parentsStateComboIx < numStates; parentsStateComboIx++) {
					NodeProbabilities cprd = cpi.getConditionalProbabilityDistribution(qcix, parentsStateComboIx);
					// LOG.debug(" cp="+cpOn+" for
					// states="+parentsStateComboIx);
					Map<Integer, Character> psm = cpi.getParentsToStateMapping(qcix, parentsStateComboIx);
					Set<Integer> onPixs = new HashSet<Integer>(); // set of
																	// parents
																	// in On
																	// state
					...
					for (int i = 0; i < pixs.size(); i++) {
						...
						if (prParentInState < 0) {
							throw new IncoherentStateException(
									"Invalid probability of " + parentState + " = " + prParentInState);
						}
				...
				LOG.info("Calculated probability for " + qcix + " ie " + kb.getClassId(qcix) + " On= "
						+ sumParentOnProbs + " Off= " + sumParentOffProbs);
