 ...
 * 
 * An OWL reasoner is used. This guarantees the graphs is a DAG. (equivalence
 * sets are mapped to the same node. subclass is mapped to DAG edges).
 * 
...
public class BMKnowledgeBaseOWLAPIImpl implements BMKnowledgeBase {
	 ...
	 * @param rf
	 */
	public BMKnowledgeBaseOWLAPIImpl(OWLOntology owlOntology, OWLOntology owlDataOntology, OWLReasonerFactory rf,
			CurieUtil curieUtil) {
		super();
	 ...
	 * @return
	 */
	public static BMKnowledgeBase create(OWLOntology owlOntology, OWLOntology owlDataOntology, OWLReasonerFactory rf,
			CurieUtil curieUtil) {
	...
	private void createMap() {
		...
		for (OWLClass c : classesInSignature) {
			...
			// note that if frequency is not tied, then depth/ancestors should
			// make
			// no overall difference - we ensure this by taking the proportion
			// of
			// ancestor nodes divided by number of classes (there are always
			// equal
		...
		// Order class nodes such that LOW frequencies (HIGH Information
		// Content)
	...
	private void storeInferences() {
		...
		// the store method is called redundantly. This is unlikely to affect
		// performance,
		...
		for (OWLNamedIndividual i : individualsInSignature) {
			...
			for (OWLClass c : owlReasoner.getTypes(i, false).getFlattened()) {
				...
				/*
				 * for (OWLDisjointClassesAxiom dca :
				 * owlOntology.getDisjointClassesAxioms(c)) { for
				 * (OWLClassExpression dc : dca.getClassExpressionsMinus(c)) {
				 * LOG.info(i+" Type: "+c+" DisjointWith: "+dc);
				 * ncs.addAll(getIntegersForClassSet(owlReasoner.getSubClasses(
				 * dc, false)));
				 * ncs.add(getIndexForClassNode(owlReasoner.getEquivalentClasses
				 * (dc))); ncsDirect.add(getIndexForClassNode(owlReasoner.
				 * getEquivalentClasses(dc))); } } for
				 ...
				 * ncs.add(getIndexForClassNode(owlReasoner.getEquivalentClasses
				 * (dc))); ncsDirect.add(getIndexForClassNode(owlReasoner.
				 * getEquivalentClasses(dc)));
				 * 
				 * } } }
				 */
			}

			ontoEWAHStore.setNegatedTypes(individualIndex, ncs); // TODO -
																	// determine
																	// if
																	// storing
																	// all
	...
	// TODO - complete this
	// TODO - separate this out as it is not an OWLAPI model. Maybe sparql is
	// overkill here?
	...
	public EWAHCompressedBitmap getIndividualsBM(int classIndex) {
		...
		// combining individuals; it is too expensive to store all inferred inds
		// by class
	...
	/**
	 * Each set of equivalent classes (a class node) is mapped to a unique
	 * integer
	 * 
	 * @param n
	...
	/**
	 * Each set of same individuals (an individual node) is mapped to a unique
	 * integer
	 * 
	 * @param n
	 * @return integer representation of class node
	...
	/**
	 * @param c
	 * @return Bitmap representation of set of superclasses of c (direct and
	 *         indirect)
	 */
	...
	/**
	 * @param clsSet
	 * @return union of all superClasses (direct and indirect) of any input
	 *         class
	 */
	...
	/**
	 * @param id
	 * @return bitmap representation of all (direct and indirect) instantiated
	 *         classes
	 */
	 ...
	 * @param id
	 * @return bitmap representation of all (direct and indirect) instantiated
	 *         classes that are subclasses of classId
	 */
