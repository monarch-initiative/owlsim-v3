 ...
 *
 */
public class NaiveBayesFixedWeightThreeStateProfileMatcherTest extends AbstractProfileMatcherTest {

	private Logger LOG = Logger.getLogger(NaiveBayesFixedWeightThreeStateProfileMatcherTest.class);
	...
	public void testBasicWithNegation() throws Exception {
		...
		for (String i : kb.getIndividualIdsInSignature()) {
			...
			eval.writeJsonTo("target/naivebfw3-test-results-"+fn+".json");
	...
	public void testExampleWithNegation() throws Exception {
		...
		TestQuery tq =  new TestQuery(pq, i, 1); // self should always be ranked first
		String fn = i.replaceAll(".*/", "");
		eval.writeJsonTo("target/naivebfw3-neg-extra-test-results-"+fn+".json");
		Assert.assertTrue(eval.evaluateTestQuery(profileMatcher, tq));
